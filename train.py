...
def validate(
    ...
    correct_chars = sum(sum(p == g for p, g in zip(pred, gt)) for pred, gt in zip(all_preds, all_gts))
    char_acc = correct_chars / total_chars if total_chars > 0 else 0.0
    word_acc = sum(p == g for p, g in zip(all_preds, all_gts)) / len(all_gts) if all_gts else 0.0
    print(f"🔸 Loss: {avg_loss:.4f}")
...
# ♻️ Checkpoint Loader
########################################
def load_checkpoint(model: nn.Module, optimizer: Optimizer, checkpoint_path: str) -> int:
